...
class NextViT(nn.Module):
    ...
    def forward(self, batch):
        ...
        for b_index, d_num in enumerate(self.depths):
            ...
            aux_x = self.aux_bn[b_index](x)
            aux_x = self.avgpool(aux_x)
            aux_x = torch.flatten(aux_x, 1)
            aux_x = self.aux_ln[b_index](aux_x)
            out['loss'] = 0.25 * get_loss(aux_x, batch['gt'])
        
        h, w = t_feats[0].shape[-2:]
        
        for i in range(4):
            t_feats[i] = nn.functional.interpolate(t_feats[i], size=(h, w), mode='nearest')
...
# batch = {
#     'images': torch.rand(2, 3, 1536, 960),
#     'gt': torch.rand(2, 1)
