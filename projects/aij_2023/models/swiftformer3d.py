...
def stem(in_chs, out_chs):
    ...
    return nn.Sequential(
        nn.Conv3d(in_chs, out_chs // 2, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1)),
        nn.BatchNorm3d(out_chs // 2),
        nn.ReLU(),
        nn.Conv3d(out_chs // 2, out_chs, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1)),
...
class Embedding(nn.Module):
    ...
    def __init__(self, patch_size=16, stride=16, padding=0,
        ...
        self.norm = nn.BatchNorm3d(num_features=embed_dim, eps=1e-6, affine=True)
...
class ConvEncoder(nn.Module):
    ...
    def __init__(self, dim, hidden_dim=64, kernel_size=3, drop_path=0., use_layer_scale=True):
        ...
        self.norm = nn.BatchNorm3d(num_features=dim, eps=1e-6, affine=True)
        ...
        if use_layer_scale:
            self.layer_scale = nn.Parameter(torch.ones(dim).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1), requires_grad=True)
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None,
        ...
        self.norm1 = nn.BatchNorm3d(num_features=in_features, eps=1e-6, affine=True)
...
class SwiftFormerLocalRepresentation(nn.Module):
    ...
    def __init__(self, dim, kernel_size=3, drop_path=0.0, use_layer_scale=True):
        ...
        self.dwconv = nn.Conv3d(dim, dim, kernel_size=kernel_size, padding=kernel_size // 2, groups=dim)
        self.norm = nn.BatchNorm3d(num_features=dim, eps=1e-6, affine=True)
        self.pwconv1 = nn.Conv3d(dim, dim, kernel_size=1)
        self.act = nn.GELU()
        self.pwconv2 = nn.Conv3d(dim, dim, kernel_size=1)
        self.drop_path = DropPath(drop_path) if drop_path > 0. \
            else nn.Identity()
        self.use_layer_scale = use_layer_scale
        if use_layer_scale:
...
class SwiftFormerEncoder(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4.,
        ...
        if use_layer_scale:
            self.layer_scale_1 = nn.Parameter(
                layer_scale_init_value * torch.ones(dim).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1), requires_grad=True)
    ...
    def forward(self, x):
        ...
        if self.use_layer_scale:
            x = x + self.drop_path(
                self.layer_scale_1 * self.attn(x.permute(0, 2, 3, 4, 1).reshape(B * F, H * W, C)).reshape(B, F, H, W, C).permute(
                    0, 4, 1, 2, 3))
            x = x + self.drop_path(self.layer_scale_2 * self.linear(x))

        else:
            x = x + self.drop_path(
                self.attn(x.permute(0, 2, 3, 4, 1).reshape(B * F, H * W, C)).reshape(B, F, H, W, C).permute(0, 4, 1, 2, 3))
...
class SwiftFormer3D(nn.Module):
    def __init__(self, arch='XS', temporal_indices=None,
        ...
        self.norm = nn.BatchNorm3d(num_features=embed_dims[-1], eps=1e-6, affine=True)
        
        self.out_conv = Conv3DBlock(frame_count=frame_count, channels=embed_dims[-1])
        self.out_norm = nn.BatchNorm3d(num_features=embed_dims[-1], eps=1e-6, affine=True)
    ...
    def load_pretrain(self, ckpt_path, ignore_layers):
        ...
        print(f'LOADED PRETRAIN SWIFTFORMER | MISSING KEYS: {len(m)} | UNEXPECTED KEYS: {len(u)}')
